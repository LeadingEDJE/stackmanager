...
def test_get_config_single():
    config = stackmanager.packager.get_config('python3.9', os.path.join(functions_dir, 'python'))
...
def test_build_lambda(capsys, monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        stackmanager.packager.build_lambda(os.path.join(functions_dir, 'python'), output_dir, 'python3.9', 'test')

        assert os.path.isfile(os.path.join(output_dir, 'test.zip'))

    captured = capsys.readouterr()
    assert 'Building python3.9 Lambda function from' in captured.out
